...
class Dataset:
  ...
  def get_fixed_point_along_route(self, row):
    ...
    for i in range(self.n_fixed_points):
      if i * avg_number >= len(points_list):
        curr_point = points_list[-1]
      else:
...
class RVSDataset(Dataset):
  def __init__(
    ...
    dev_graph_embed_path: str = "",
    test_graph_embed_path: str = "",
    size_of_train_split: int = 100000,
    ...
    self.train_raw = self.load_data(
      data_dir, train_region.lower(), lines=True, size_of_train_split=size_of_train_split)
    self.dev_raw = self.load_data(
      data_dir, dev_region.lower(), lines=True, size_of_train_split=size_of_train_split)
    self.test_raw = self.load_data(
      data_dir, test_region.lower(), lines=True, size_of_train_split=size_of_train_split)
  ...
  def load_data(self, data_dir: str, ds_set: str, lines: bool, size_of_train_split: int):
    ...
    max_sample = size_of_train_split if ds.shape[0]>size_of_train_split else ds.shape[0]
...
class WikiGeoDataset(Dataset):
  ...
  def load_data(self, data_dir: str, ds_set: str, lines: bool):
    ...
    ds = pd.read_json(ds_path, lines=lines)
    ds['instructions'] = ds['text']
    ds['end_point'] = ds['ref_point'].apply(gutil.point_from_str_coord_yx)
    ds['start_point'] = ds['ref_point'].apply(gutil.point_from_str_coord_yx)
...
class SyntheticDataset(Dataset):
  ...
  def load_data(self, data_dir: str, ds_set: str, lines: bool, size_of_train_split: int):
    ...
    ds = ds.iloc[:max_sample]
    logging.info(f"Resized {ds_set}-set to {ds.shape[0]}")
