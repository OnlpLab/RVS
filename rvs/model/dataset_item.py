...
class TextGeoDataset:
  ...
  @classmethod
  def load(
    cls, dataset_dir, model_type, s2_level, train_region, dev_region, test_region, infer_only):
    if model_type:
  ...
  def __init__(self, region: str, data: pd.DataFrame, s2level: int,
    ...
    if graph_embed_file:
      self.graph_embed_end = data['cellid'].apply(
        lambda cell: util.get_valid_graph_embed(self.graph_embed_file, str(cell))).tolist()
