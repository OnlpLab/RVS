...
NON_PROMI_STREET_NAMES = ['poi','',np.nan]
...
class Walker:
  ...
  def get_number_intersections_past(self,
    ...
    if pivot_goal_route.shape[0]<=4:
      return None


    edges_in_pivot_goal_route_osmid = pivot_goal_route['osmid'].apply(
      lambda x: set(
      self.map.edges[
        (self.map.edges['u'] == x) & (~self.map.edges['name'].isin(NON_PROMI_STREET_NAMES))]['osmid'].tolist()))   

    assert pivot_goal_route.shape[0]==edges_in_pivot_goal_route_osmid.shape[0]



    # Make sure it is a straight line

    pivot_streets = edges_in_pivot_goal_route_osmid.iloc[1]
    goal_streets = edges_in_pivot_goal_route_osmid.iloc[-2]
    common_streets = pivot_streets & goal_streets

    if not common_streets:
      return None
        
    assert len(common_streets)==1

    unique_streets = []
    for idx, street_ids in enumerate(edges_in_pivot_goal_route_osmid.tolist()):
      if idx==0:
        continue
      current_instersected_streets = street_ids - common_streets
      if len(current_instersected_streets)>0:
        for street in current_instersected_streets:
          if street not in unique_streets:
            unique_streets.append(street)

    unique_street_names = self.map.edges[
      (self.map.edges['osmid'].isin(tuple(unique_streets)) & (
      ~self.map.edges['name'].isin(NON_PROMI_STREET_NAMES)))]['name'].unique().tolist() 

    number_intersection = len(unique_street_names)
...
def load_entities(path: str, n_samples=None) -> Sequence[geo_item.GeoEntity]:
  ...
  for landmark_type in LANDMARK_TYPES:
    geo_types_all[landmark_type] = gpd.read_file(path, layer=landmark_type, rows=n_samples)

  geo_types_all['route'] = gpd.read_file(path, layer='path_features', row=n_samples)['geometry']
  geo_types_all['path_features'] = gpd.read_file(path, layer='path_features', row=n_samples)
  geo_entities = []
  ...
  for row_idx in range(geo_types_all[LANDMARK_TYPES[0]].shape[0]):
    ...
    for landmark_type in LANDMARK_TYPES:
      try:
        ...
        is_process = False
        logging.info(f"Skip over index {row_idx}")
        break
    if not is_process:
      continue
