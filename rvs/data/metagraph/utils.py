...
IMPORTANT_POI_METADATA_NAMES = [
  "name", "brand", "shop", "tourism", "amenity"
]
...
def construct_metagraph(region: Region,
  ...
  null_idx = osm_map.poi[osm_map.poi['osmid'].isnull()==True].index.values.tolist()
...
def draw_graph(metagraph, path_to_save):
  
  # Define the node colors
  # Get the node types

  node_types = [data['type'] for _, data in metagraph.nodes(data=True)]

  node_colors = []
  for node_type in node_types:
    if node_type == TYPE_S2:
      node_colors.append('red')
    elif node_type == TYPE_PROJECTED_POI:
      node_colors.append('green')
    else:
      node_colors.append('blue')

  # Get the positions of the nodes
  pos = nx.spring_layout(metagraph)

  nx.draw(metagraph, pos, node_color=node_colors)

  node_labels = {}
  for node, data in metagraph.nodes(data=True):
    label = ""
    for col in IMPORTANT_POI_METADATA_NAMES:
      if col in data:
        label += data[col]
    node_labels[node] = label

  nx.draw_networkx_labels(metagraph, pos, labels=node_labels)

  
  logging.info(f"Drew graph => {path_to_save}")
